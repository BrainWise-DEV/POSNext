        ...
        <div v-else-if="closingData" class="flex flex-col gap-3 md:gap-6">
          ...
          <div v-if="shouldShowSummary" class="bg-white border border-gray-200 rounded-lg p-3 md:p-6 shadow-sm">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4">
              ...
              <!-- POS Expenses -->
              <div v-if="hasExpenses" class="text-start bg-amber-50 border border-amber-200 rounded-lg p-3 md:p-4">
                <div class="text-amber-700 text-xs uppercase font-medium mb-1">{{ __('POS Expenses') }}</div>
                <div class="text-lg md:text-2xl font-bold text-amber-900 mb-0.5 md:mb-1 truncate">-{{ formatCurrency(closingData.expenses_total) }}</div>
                <div class="text-amber-700 text-xs">{{ __('{0} expenses', [closingData.expenses_count]) }}</div>
              </div>
              ...
              <!-- Net Cash Impact -->
              <div v-if="hasExpenses" class="text-start bg-purple-50 border border-purple-200 rounded-lg p-3 md:p-4">
                <div class="text-purple-700 text-xs uppercase font-medium mb-1">{{ __('Net Cash Impact') }}</div>
                <div class="text-lg md:text-2xl font-bold text-purple-900 mb-0.5 md:mb-1 truncate">{{ formatCurrency(netCashImpact) }}</div>
                <div class="text-purple-700 text-xs">{{ __('Sales - Returns - Expenses') }}</div>
              </div>
          ...
          <div v-if="hasExpenses" class="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
            <div class="px-3 py-3 md:px-6 md:py-4 border-b border-gray-200 bg-amber-50">
              <h3 class="text-sm md:text-lg font-semibold text-gray-900 text-start">{{ __('POS Expenses') }}</h3>
            ...
            <div class="overflow-x-auto">
              <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                  <tr>
                    <th class="px-4 py-3 text-start text-xs font-medium text-gray-500 uppercase">{{ __('Expense Account') }}</th>
                    <th class="px-4 py-3 text-start text-xs font-medium text-gray-500 uppercase">{{ __('Amount') }}</th>
                    <th class="px-4 py-3 text-start text-xs font-medium text-gray-500 uppercase">{{ __('Employee') }}</th>
                    <th class="px-4 py-3 text-start text-xs font-medium text-gray-500 uppercase">{{ __('Remarks') }}</th>
                  </tr>
                </thead>
                <tbody class="divide-y divide-gray-200">
                  <tr v-for="(expense, idx) in closingData.pos_expenses" :key="idx">
                    <td class="px-4 py-3 text-sm text-gray-900 text-start">{{ expense.expense_account }}</td>
                    <td class="px-4 py-3 text-sm font-semibold text-amber-800 text-start">{{ formatCurrency(expense.amount) }}</td>
                    <td class="px-4 py-3 text-sm text-gray-600 text-start">{{ expense.employee || __('N/A') }}</td>
                    <td class="px-4 py-3 text-sm text-gray-600 text-start">{{ expense.remarks || __('N/A') }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
