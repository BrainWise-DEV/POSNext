	...
	<div class="flex flex-col h-full bg-gray-50">
		...
		>
			<div class="text-center py-8">
				...
				<p
					v-if="searchTerm || selectedItemGroup"
					class="mt-2 text-xs font-medium text-gray-700"
				>
					<span v-if="searchTerm && selectedItemGroup">{{
						__("No results for {0} in {1}", [searchTerm, selectedItemGroup])
					}}</span>
					<span v-else-if="selectedItemGroup">{{
						__("No results in {0}", [selectedItemGroup])
					}}</span>
