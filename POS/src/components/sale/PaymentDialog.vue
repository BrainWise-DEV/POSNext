				...
				>
					...
					<div :class="isSmallMobile ? 'mb-1' : 'mb-1.5 lg:mb-3'">
						...
						>
							...
							<!-- Receivable Accounts (Pay on Account to a specific receivable) -->
...
// "Pay on Receivable Account": selectable AR accounts for the invoice's debit_to
...
const canComplete = computed(() => {
	...
	// "Pay on Receivable Account": with a receivable account chosen and credit sales
	// enabled, Complete Payment finalizes the sale with the unpaid balance left
	// outstanding on that account — whether the cashier paid part now (partial) or
	// nothing at all (full credit). No payment row is required in that case.
...
function toggleReceivableAccount(acc) {
	...
	log.debug(
		"[PaymentDialog] Receivable account:",
		selectedReceivableAccount.value || "(default)",
	);
}
...
function completePayment() {
	...
	const paymentData = {
		...
		// "Pay on Receivable Account": route any outstanding to the chosen AR (else default).
		// With no payment row it's a full credit sale, so flag it to allow the no-payment
		// submit (backend re-checks the allow_credit_sale gate).
		receivable_account: selectedReceivableAccount.value || null,
		is_credit_sale:
			!!selectedReceivableAccount.value && paymentEntries.value.length === 0,
	};
