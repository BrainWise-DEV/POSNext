...
export const useItemSearchStore = defineStore("itemSearch", () => {
	...
	async function searchByBarcode(barcode) {
		...
		const tryCache = async () => {
			try {
				const cached = await offlineWorker.searchCachedItems(trimmed, 1)
				return cached?.[0] || null
			} catch (cacheErr) {
				log.warn("Offline barcode lookup failed", cacheErr)
				return null
			}
		}

		// Offline: cache only — no server reachable.
		if (isOffline()) {
			log.debug("Offline barcode lookup", { barcode: trimmed })
			return await tryCache()
		}

		// Online: try server first for fresh price/stock; fall back to cache on failure.
		try {
			...
			if (item) return item
			// Server returned nothing — try cache (item may exist locally but not match server filters).
			return await tryCache()
		} catch (error) {
			log.warn(
				"Server barcode lookup failed, falling back to offline cache",
				error,
			)
