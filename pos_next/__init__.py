...
# Call-time guards in the patched functions defer to posnext_promotions when that app is
# installed on the *current* site (import-time checks freeze the wrong site on multi-tenant workers).
try:
...
try:
	...
	from pos_next.overrides.pricing_rule import patch_apply_price_discount_rule

	patch_apply_price_discount_rule(_erpnext_pricing_rule)
except Exception:
	if frappe:
