...
def get_returnable_invoices(limit: int = 50, pos_profile: str | None = None):
	...
	query = (
		...
		.left_join(ret_si)
		.on((ret_si.return_against == si.name) & (ret_si.docstatus == 1) & (ret_si.is_return == 1))
		.left_join(ret_item)
		.on(
		...
		.select(
			...
			si.grand_total,
			si.status,
			Coalesce(Sum(Case().when(ret_item.qty.isnotnull(), Abs(ret_item.qty)).else_(0)), 0).as_(
				"total_returned_qty"
			),
			Coalesce(Sum(Case().when(si_item.qty.isnotnull(), si_item.qty).else_(0)), 0).as_(
				"total_original_qty"
			),
		)
