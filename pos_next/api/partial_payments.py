	...
	if payment_account:
		...
		try:
			...
			frappe.log_error(
				...
				message=f"Mode of Payment: {mode_of_payment}, Company: {invoice.company}, Error: {e!s}",
			)
			frappe.throw(_("Could not determine payment account. Please specify payment_account parameter."))
		...
		for pe_name in payment_entries_created:
			try:
				...
				frappe.log_error(
					...
					message=f"Original Error: {e!s}\nRollback Error: {cancel_error!s}\n\n{frappe.get_traceback()}",
				)
