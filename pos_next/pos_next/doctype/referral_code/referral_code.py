...
def apply_referral_code(referral_code, referee_customer):
	...
	try:
		...
		frappe.log_error(
			title="Referrer Coupon Generation Failed", message=f"Failed to generate referrer coupon: {str(e)}"
		)
	...
	try:
		...
		frappe.log_error(
			title="Referee Coupon Generation Failed", message=f"Failed to generate referee coupon: {str(e)}"
		)
